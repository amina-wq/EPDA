...
<div class="container">
            ...
            <div class="stat-value">
                <c:choose>
                    <c:when test="${dashboardStats.totalStudents > 0}">
                        <fmt:formatNumber value="${(dashboardStats.totalStudents - dashboardStats.ineligibleCount) / dashboardStats.totalStudents * 100}" maxFractionDigits="1"/>%
                    </c:when>
    ...
    <c:choose>
        <c:when test="${empty ineligibleStudents}">
        ...
        <c:otherwise>
            <table>
                ...
                <tbody>
                    <c:forEach items="${ineligibleStudents}" var="item">
                        <tr>
                            <td>
                                <strong>
                                    <a href="${pageContext.request.contextPath}/student_recovery?studentId=${item.student.studentId}" 
                                       class="student-link" 
                            ...
                            <td>
                                <span class="badge ${item.cgpa < 2.0 ? 'badge-danger' : 'badge-warning'}">
                                    <fmt:formatNumber value="${item.cgpa}" pattern="0.00"/>
                                </span>
                            </td>
                            <td>
                                <span class="badge ${item.failedCount > 3 ? 'badge-danger' : 'badge-warning'}">
                                    ${item.failedCount}
                                </span>
                            </td>
                            <td>
                            ...
                            <td>
							    <form action="${pageContext.request.contextPath}/confirm_registration" method="post" style="margin:0;">
    ...
    <div style="margin-top: 30px; padding: 20px; background: #e8f5e9; border-radius: 8px;">
        ...
        <p>Students must meet BOTH of the following criteria to be eligible for progression:</p>
        <ul>
            <li><strong>CGPA ≥ 2.0</strong> - Calculated as (Total Grade Points) / (Total Credit Hours)</li>
            <li><strong>Failed courses ≤ 3</strong> - Students with more than 3 failed courses are not eligible</li>
        </ul>
        <p style="margin-bottom: 0; color: #666; font-style: italic;">
            Note: Administrators can override eligibility by confirming registration for individual students.
        </p>
    </div>
</div>
